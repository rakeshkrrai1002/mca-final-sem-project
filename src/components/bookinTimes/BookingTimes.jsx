...
const BookingTimes = () => {
  ...
  const fetchBookedSlotsAndDates = async () => {
    try {
      const response = await fetch(
        `http://${api}/get_beds_and_reservation_info/${selectedFloor.floor_id}`
      );
  ...
  const groupDormitoryBeds = (dormitoryRoom) => {
    ...
    dormitoryRoom.beds.forEach((bed) => {
      ...
      const displayName = `${dormitoryRoom.group_key} - Bed ${bedConfig?.bed_no || bed.bed_id
        }`;
                            ...
                            <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-12 gap-2">
                              {timeSlots.map((time) => {
                                ...
                                const isSelected = selectedTimes.has(
                                  `${room.group_key}-${formatDate(currentDate)}-${time}`
                                );

                                const now = new Date();
                                const viewedDate = new Date(currentDate);
                                const isToday = viewedDate.toDateString() === now.toDateString();
                                const isPastDay = viewedDate < now && !isToday;
                                const slotHour = parseInt(time.split(":")[0], 10);
                                const currentHour = now.getHours();

                                // Check if this is a past time slot (either past day or past time on today)
                                const isPastTimeSlot = isPastDay || (isToday && slotHour < currentHour);

                                // A slot is bookable if it's not in the past, unless it's already booked
                                const isBookable = !isPastTimeSlot || status === "booked";

                                return (
                                  <button
                                    key={`${room.group_key}-${currentDate.toISOString()}-${time}`}
                                    onClick={() => {
                                      if (!isBookable && status !== "booked") {
                                        alert(
                                          "Booking is not allowed for past dates or times."
                                        );
                                        return;
                                      }
                                      if (status === "booked") {
                                    ...
                                    className={`py-2 px-1 rounded-lg text-xs font-medium transition-all ${status === "booked"
                                        ? "bg-gray-200 text-gray-400 cursor-pointer" // Booked slots stay as booked regardless of time
                                        : isPastTimeSlot
                                          ? "bg-white text-gray-300 border border-gray-200" // Past slots style
                                          : isSelected
                                            ? "bg-ezzstay-base text-white shadow-sm" // Selected slots
                                            : "bg-purple-100 text-ezzstay-base hover:bg-purple-200" // Available slots
                                      }`}
                                  >
                                    {time}
                                  </button>
  ...
  const isSelected = selectedTimes.has(
    `${groupKey}-${formatDate(currentDate)}-${time}`
  );
  ...
  return (
    <button
      ...
      className={`py-2 px-1 rounded-lg text-xs font-medium transition-all ${
        status === "booked"
          ? "bg-gray-200 text-gray-400 cursor-pointer" // Booked slots maintain their appearance
          : isPastTimeSlot
            ? "bg-white text-gray-300 border border-gray-200" // Past slots style
            : isSelected
              ? "bg-ezzstay-base text-white shadow-sm" // Selected slots
    ...
    >
      ...
      {pnrModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto relative">
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div
            className="max-h-screen overflow-y-auto py-10 relative scrollbar-hide"
            style={{ width: "400px", scrollbarWidth: "none" }}
          >
            <div className="bg-white rounded-xl p-6 relative">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className={`w-full p-2 border rounded ${user && user.mobile ? "bg-gray-100" : ""
                    }`}
